...
class NavBar extends Component {
  ...
  getAnchorPoints() {
    ...
    /* If max-scroll is less than the pixel-depth of Section 3, 
    then adjust the pixel-depth of Section 3 to be 50px higher than max scroll depth 
    ... allowing it to be an active destination */
  ...
  render() {
    ...
    for (var key in this.sections) {
      menuItems.push(
        <NavBarItem itemName={key} active={this.state.activeMenuItem[key]} />
      );
    }
    return (
      <nav id="nav">
        <ul className="Nav-Bar">{menuItems}</ul>
      </nav>
    );
  }
}
